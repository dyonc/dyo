...
export default function Metatags({ tweets }: { tweets: any }) {
  const [url, setUrl] = useState("https://github.com/dyonc/dyo");
    ...
    <HomeLayout
      meta={
        <Meta
          ...
          description="Dyo's Metatags API is a free & simple API to retrieve meta & OG tags from a URL, powered by Vercel Edge Functions."
        />
      }
    >
      <Background />
      <div className="z-10 mx-auto my-10 flex max-w-md flex-col space-y-5 px-2.5 text-center sm:max-w-lg sm:px-0 lg:mb-28">
        <a
          href="https://twitter.com/DyonicLTD/status/1595459224498233347"
          target="_blank"
          rel="noreferrer"
        ...
        <button
          ...
          onClick={() => {
            navigator.clipboard.writeText(
              `https://api.dyo.at/metatags?url=${getUrlFromString(url)}`,
            );
            setCopied(true);
            setTimeout(() => {
              setCopied(false);
        ...
        <a
          href="https://dyo.at/metatags-code"
          target="_blank"
          rel="noreferrer"
...
export async function getStaticProps() {
  const tweets = await getTweetsMetadata(["1594089062545391617"]);
